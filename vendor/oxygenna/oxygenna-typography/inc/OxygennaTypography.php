...
class OxygennaTypography
{
    ...
    { ?>
        <div class="error">
            <p><?php _e('Looks like you have the <strong>Oxygenna Typography Plugin</strong> Installed - this is no longer needed please delete it <a href="' . admin_url('plugins.php') . '">here on your plugins page</a>', 'angle-admin-td'); ?></p>
        </div>
    <?php
    }
    ...
    public function google_fetch_fonts_list()
    {
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'google-fetch-fonts-nonce')) {
                ...
                $resp->message = __('Could not verify nonce', 'angle-admin-td');
            }
        }
    ...
    public function google_update_font_list()
    {
        ...
        if (is_wp_error($response)) {
            $resp->status = false;
            $resp->message = __('Error fetching Google Font List', 'angle-admin-td');
        } else {
    ...
    public function font_modal()
    {
        if (isset($_GET['nonce'])) {
            if (wp_verify_nonce($_GET['nonce'], 'font-modal')) {
                ...
                $save_button_text = $isNew ? __('Add font', 'angle-admin-td') : __('Update Font', 'angle-admin-td');
                ...
                $elements = array(
                    'body'       => __('All (body tag)', 'angle-admin-td'),
                    'headings'   => __('Headings (h1-h6 tags)', 'angle-admin-td'),
                    'forms'      => __('Forms (all input tags)', 'angle-admin-td'),
                    'blockquote' => __('Block Quote (blockquote tag)', 'angle-admin-td'),
                );
    ...
    public function fontstack_save()
    {
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'update-fontstack')) {
                ...
                // set response to ok
                $resp->status = true;
                $resp->message = __('Fontsack Updated Successfully', 'angle-admin-td');
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'default-fonts')) {
                ...
                if (false !== $google_fonts) {
                    ...
                    $resp->message = __('Default Fonts Installed Successfully', 'angle-admin-td');
                } else {
                    $resp->message = __('No Google Fonts Installed. Please go to ' . THEME_NAME . ' -> Fonts and update your google fonts', 'angle-admin-td');
                }
            }
        }
