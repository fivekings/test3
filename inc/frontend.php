...
function oxy_load_scripts() {
    ...
    wp_enqueue_style( THEME_SHORT.'-theme', OXY_THEME_URI . 'assets/css/theme.css', array( THEME_SHORT.'-bootstrap', 'wp-mediaelement' ), false, 'all' );
...
function oxy_create_flexslider( $slug_or_ids, $options = array(), $echo = true ) {
    ...
    $output = '';
    $output .= '<div id="' . $id . '" class="flexslider feature-slider"'.$flex_itemwidth.' data-flex-animation="'.$animation.'" data-flex-controlsalign="'.$controlsalign.'" data-flex-controlsposition="'.$controlsposition.'" data-flex-directions="'.$directionnav.'" data-flex-directions-type="'.$directionnavtype.'" data-flex-speed="'.$speed.'" data-flex-controls="'.$showcontrols.'" data-flex-slideshow="' . $autostart . '" data-flex-duration="'.$duration.'" data-flex-captionvertical="'.$captions_vertical.'" data-flex-captionhorizontal="'.$captions_horizontal.'" data-flex-sliderdirection="'.$direction.'">';
...
function oxy_navbar_class() {
    ...
    $classes[] = $oxy_theme_options['header_capitalization'] == 'off'? 'text-lowercase' : 'text-caps';
